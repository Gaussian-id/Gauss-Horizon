...
fn strip_large_value_marker_prefix(column: &str) -> Option<&str> {
    let prefix = crate::sql_dialect::CHIRON_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX;
...
fn external_driver_preview_fallback_sql(sql: &str) -> Option<String> {
    ...
    if !projection.contains(crate::sql_dialect::CHIRON_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX) {
        return None;
    }
...
"#
    ...
    async fn native_pre_dispatch_cancellation_stays_typed() {
        ...
        let backend_error = error.into_backend_error();
        assert_eq!(backend_error.code(), "Chiron Horizon-JDBC-2003");
    ...
    async fn mysql_pipelined_batch_reports_the_first_failed_statement() {
        ...
        assert_eq!(results[1].statement_index, Some(1));
        assert!(results[1].execution_error);
        assert_eq!(results[1].error.as_ref().map(|error| error.code()), Some("Chiron Horizon-JDBC-4001"));
        assert_eq!(
            progress_events
                .lock()
                .unwrap()
    ...
    fn query_execution_error_preserves_structured_catalog_identity() {
        ...
        assert_eq!(error.into_backend_error().code(), "Chiron Horizon-JDBC-1002");
    }
    ...
    fn query_timeout_preserves_timeout_catalog_identity_and_detail() {
        ...
        assert_eq!(backend_error.code(), "Chiron Horizon-JDBC-2002");
        assert_eq!(
    ...
    fn postgres_transaction_uses_driver_fact_not_error_wording() {
        ...
        assert_eq!(error.into_backend_error().code(), "Chiron Horizon-JDBC-4001");
    }
    ...
    async fn external_driver_preview_retry_preserves_marker_truncation() {
        ...
        let marker = format!("{}T_1", crate::sql_dialect::CHIRON_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX);
        let sql = format!(
...
"#
    ...
    fn extracts_server_large_value_markers_and_restores_source_columns() {
        let mut result = db::QueryResult {
            columns: vec![
                "id".to_string(),
                "payload".to_string(),
                format!("{}T_1", crate::sql_dialect::CHIRON_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX),
                "note".to_string(),
            ],
    ...
    fn truncates_server_binary_preview_after_the_configured_byte_count() {
        let mut result = db::QueryResult {
            columns: vec![
                "raw_value".to_string(),
                format!("{}B_0", crate::sql_dialect::CHIRON_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX),
            ],
    ...
    fn extracts_deferred_oracle_clob_markers_without_changing_placeholder() {
        let mut result = db::QueryResult {
            columns: vec![
                "id".to_string(),
                "payload".to_string(),
                format!("{}C_1", crate::sql_dialect::CHIRON_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX),
            ],
    ...
    fn restores_pgvector_previews_as_arrays_and_marks_only_truncated_values() {
        let marker = format!("{}V_0", crate::sql_dialect::CHIRON_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX);
        let mut result = db::QueryResult {
    ...
    fn restores_server_preview_types_without_rows() {
        let mut result = db::QueryResult {
            columns: vec![
                "embedding".to_string(),
                format!("{}V_0", crate::sql_dialect::CHIRON_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX),
                "metadata".to_string(),
                format!("{}K_1", crate::sql_dialect::CHIRON_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX),
            ],
    ...
    fn ordinary_queries_preserve_columns_that_resemble_preview_markers() {
        let marker = format!("{}0", crate::sql_dialect::CHIRON_HORIZON_LARGE_VALUE_BYTES_COLUMN_PREFIX);
