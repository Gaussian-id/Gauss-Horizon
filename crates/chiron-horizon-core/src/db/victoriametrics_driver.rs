...
fn expand_cert_path(path: &str) -> String {
    let home = || crate::legacy::var(if cfg!(windows) { "USERPROFILE" } else { "HOME" }).ok();
    ...
    if let Some(rest) = path.strip_prefix("%USERPROFILE%") {
        if let Ok(home) = crate::legacy::var("USERPROFILE") {
            return format!("{home}{rest}");
        }
    }
    path.to_string()
...
mod tests {
    ...
    #[tokio::test]
    #[ignore = "requires CHIRON_HORIZON_LIVE_VICTORIAMETRICS_* env vars pointing at a VictoriaMetrics endpoint"]
    async fn live_connection_metadata_and_query() {
        let url = crate::legacy::var("CHIRON_HORIZON_LIVE_VICTORIAMETRICS_URL")
            .expect("CHIRON_HORIZON_LIVE_VICTORIAMETRICS_URL");
        let username = crate::legacy::var("CHIRON_HORIZON_LIVE_VICTORIAMETRICS_USER").unwrap_or_default();
        let password = crate::legacy::var("CHIRON_HORIZON_LIVE_VICTORIAMETRICS_PASSWORD").unwrap_or_default();
        let api_path =
            crate::legacy::var("CHIRON_HORIZON_LIVE_VICTORIAMETRICS_API_PATH").unwrap_or_else(|_| "/prometheus".into());
        let metric = crate::legacy::var("CHIRON_HORIZON_LIVE_VICTORIAMETRICS_METRIC")
            .expect("CHIRON_HORIZON_LIVE_VICTORIAMETRICS_METRIC");
        let config: ConnectionConfig = serde_json::from_value(json!({
