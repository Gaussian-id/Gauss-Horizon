...
pub use installer::{
    ...
    PluginRollbackResult, PluginSignatureStatus, PluginTrustStore, PluginTrustedKey, CHIRON_HORIZON_PLUGIN_EXTENSION,
...
pub use manifest::{
    ...
    PluginFormFieldType, PluginManifest, PluginSchemaViewerContribution, PluginUiEntrypoint,
    ...
    PLUGIN_SCHEMA_VIEWER_SCOPES_METHOD, PLUGIN_SCHEMA_VIEWER_VIEW_METHOD, SUPPORTED_PLUGIN_HOST_API_VERSION,
...
"#,
    ...
    async fn shutdown_kills_plugin_child_process() {
        let dir = std::env::temp_dir().join(format!("chiron-horizon-plugin-shutdown-test-{}", uuid::Uuid::new_v4()));
    ...
    fn registry_lists_frontend_contributions_from_disk() {
        let root = std::env::temp_dir().join(format!("chiron-horizon-plugin-registry-test-{}", uuid::Uuid::new_v4()));
    ...
    fn registry_keeps_other_plugins_visible_when_one_manifest_is_corrupt() {
        let root =
            std::env::temp_dir().join(format!("chiron-horizon-plugin-corrupt-registry-test-{}", uuid::Uuid::new_v4()));
