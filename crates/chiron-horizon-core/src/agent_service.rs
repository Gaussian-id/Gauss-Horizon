...
/// AV scanning) and suggests restarting chiron-horizon; on POSIX returns a concise
...
fn agent_download_candidate_urls(source: DownloadSource, github_url: &str) -> Result<Vec<String>, String> {
    match source {
        DownloadSource::Official => {
            // Unit tests use a loopback server to exercise cancellation. The
            // compiled product only accepts the immutable Chiron Horizon
            // release location.
            let is_loopback_test_url = cfg!(test) && github_url.starts_with("http://127.0.0.1");
            if !github_url.starts_with(CHIRON_HORIZON_RELEASE_PREFIX) && !is_loopback_test_url {
                return Err(format!("Unsupported Chiron Horizon agent download URL: {github_url}"));
            }
            Ok(vec![github_url.to_string()])
        }
        DownloadSource::Cnb => Err("Chiron Horizon agent mirror is not available.".to_string()),
    }
}
...
pub fn local_agent_jar_candidates(db_type: &str) -> Vec<PathBuf> {
    let jar_name = format!("chiron-horizon-agent-{db_type}.jar");
...
fn local_agents_dir_candidates() -> Vec<PathBuf> {
    ...
    candidates.push(PathBuf::from("..").join("chiron-horizon-agents"));
    candidates.push(PathBuf::from("chiron-horizon-agents"));
    candidates
}
...
pub async fn fetch_registry_from_claimed(
    ...
    let urls = agent_download_candidate_urls(source, REGISTRY_PATH)?;
...
async fn fetch_registry_from_urls(
    ...
    let resp = open_download_response(&client, urls, "chiron-horizon-agent-manager", cancellations)
        .await
...
async fn open_agent_download_response(
    ...
    let mut errors = Vec::new();
    for candidate_url in agent_download_candidate_urls(source, github_url)? {
...
pub fn ensure_driver_app_version(
    ...
    Err(format!(
        "{db_type} driver {} requires Chiron Horizon {} or newer. Current Chiron Horizon version is {}.",
...
fn extract_jre_key_from_filename(name: &str) -> Option<String> {
    ...
    let rest = filename.strip_prefix("chiron-horizon-jre-").or_else(|| filename.strip_prefix("jre-"))?;
...
fn extract_db_type_from_filename(name: &str) -> Option<String> {
    let filename = name.rsplit('/').next()?;
    let rest = filename.strip_prefix("chiron-horizon-agent-")?;
...
mod agent_download_url_tests {
    ...
    #[test]
    fn official_agent_downloads_only_use_chiron_horizon_releases() {
    ...
    #[test]
    fn agent_downloads_reject_mirrors_and_non_gaussian_urls() {
        assert_eq!(
            agent_download_candidate_urls(DownloadSource::Cnb, REGISTRY_PATH).unwrap_err(),
            "Chiron Horizon agent mirror is not available."
        );
        assert!(agent_download_candidate_urls(
            DownloadSource::Official,
            "https://example.invalid/legacy-agent-registry.json",
        )
        .unwrap_err()
        .contains("Unsupported Chiron Horizon agent download URL"));
    }

    #[test]
    fn offline_jre_filename_parser_accepts_release_and_legacy_names() {
        assert_eq!(
            extract_jre_key_from_filename("jre/chiron-horizon-jre-21-macos-aarch64.tar.gz").as_deref(),
            Some("21")
        );
    ...
    fn windows_native_header_validator_checks_cpu_architecture() {
        let path = std::env::temp_dir().join(format!("chiron-horizon-agent-pe-test-{}", uuid::Uuid::new_v4()));
...
mod agent_registry_install_tests {
    ...
    fn test_manager(name: &str) -> AgentManager {
        let dir =
            std::env::temp_dir().join(format!("chiron-horizon-agent-registry-install-{name}-{}", uuid::Uuid::new_v4()));
    ...
    fn build_tar_zstd_driver_package(
        ...
        let (filename, artifact) = match kind {
            DriverArtifactKind::Jar => (
                format!("chiron-horizon-agent-{db_type}-{version}.jar"),
                serde_json::json!({
                    "jar": {
                        "url": format!("chiron-horizon-agent-{db_type}-{version}.jar"),
            ...
            DriverArtifactKind::Native => {
                ...
                let filename = format!("chiron-horizon-agent-{db_type}-{version}-{platform}{extension}");
                (
                    filename.clone(),
                    serde_json::json!({
                        "native": {
    ...
    async fn cache_test_registry(registry: AgentRegistry) {
        REGISTRY_CACHE.lock().await.insert(DownloadSource::Official, (std::time::Instant::now(), registry));
    }
    ...
    async fn ensure_agent_runtime_installs_missing_driver_and_jre() {
        ...
        let driver_url = "https://example.com/chiron-horizon-agent-mongodb.jar";
        let driver_bytes = test_agent_jar();
        let jre_version = "21.0.12";
        let jre_url = "https://example.com/chiron-horizon-jre.tar.gz";
        ...
        ensure_agent_driver_ready_from(&manager, "mongodb", DownloadSource::Official).await.unwrap();
    ...
    async fn ensure_agent_runtime_ready_fast_path_preserves_valid_old_driver() {
        ...
        let latest_url = "https://example.com/chiron-horizon-agent-mongodb-latest.jar";
        ...
        tokio::time::timeout(
            ...
            ensure_agent_driver_ready_from(&manager, "mongodb", DownloadSource::Official),
        )
        .await
    ...
    async fn ensure_agent_runtime_repairs_damaged_jre_without_upgrading_valid_driver() {
        ...
        let registry = mongodb_registry_with_jre(
            "9.9.9",
            "https://example.com/chiron-horizon-agent-mongodb-latest.jar",
    ...
    async fn concurrent_ensure_agent_runtime_installs_once() {
        ...
        let first = ensure_agent_driver_ready_from(&manager, "mongodb", DownloadSource::Official);
        let second = ensure_agent_driver_ready_from(&manager, "mongodb", DownloadSource::Official);
    ...
    async fn ensure_agent_runtime_propagates_corrupt_install_error() {
        ...
        let error = ensure_agent_driver_ready_from(&manager, "mongodb", DownloadSource::Official)
            .await
    ...
    async fn registry_install_cancel_after_cached_download_preserves_existing_driver() {
        ...
        let url = "https://example.com/chiron-horizon-agent-mongodb.jar";
    ...
    async fn registry_install_extracts_tar_zstd_java_driver_package() {
        ...
        let db_type = "dameng";
        let version = "0.2.0";
        let package_url = "https://example.com/chiron-horizon-agent-dameng.tar.zst";
    ...
    async fn batch_upgrade_preserves_successful_state_and_reports_independent_failure() {
        ...
        let oracle_url = "https://example.com/chiron-horizon-agent-oracle";
        let dameng_url = "https://example.com/chiron-horizon-agent-dameng";
        let kingbase_url = "https://example.com/chiron-horizon-agent-kingbase.jar";
    ...
    async fn batch_cancel_all_aborts_in_flight_downloads_without_persisting() {
        ...
        let native_url = format!("http://{addr}/chiron-horizon-agent-oracle");
        let registry = registry_with_native_and_legacy_jar(db_type, version, &native_url, native_bytes.len() as u64);
    ...
    async fn shared_jre_is_not_downloaded_again_after_the_first_install_persists_its_version() {
        ...
        let jre_key = DEFAULT_JRE_KEY;
        let version = "21.0.12";
        let url = "https://example.com/chiron-horizon-jre.tar.gz";
    ...
    async fn managed_jre_install_extracts_tar_zstd_archive() {
        ...
        let jre_key = DEFAULT_JRE_KEY;
        let version = "21.0.12";
        let url = "https://example.com/chiron-horizon-jre.tar.zst";
    ...
    async fn manual_import_waits_for_an_existing_driver_operation() {
        ...
        let db_type = "h2";
        let source = manager.base_dir().join("chiron-horizon-agent-h2.jar");
    ...
    async fn download_with_progress_returns_cancelled_when_token_prefired() {
        ...
        let error = download_with_progress(
            ...
            "driver",
            DownloadSource::Official,
            "https://example.com/chiron-horizon-agent-oracle",
            &dest,
            100,
            None,
            None,
    ...
    async fn concurrent_same_driver_installs_cancel_only_the_targeted_operation() {
        ...
        let install_a = tokio::spawn(async move {
            install_agent_driver_from_claimed(&manager_a, db_type, DownloadSource::Official, |_| {}, &token_a_task)
                .await
        });
        let install_b = tokio::spawn(async move {
            install_agent_driver_from_claimed(&manager_b, db_type, DownloadSource::Official, |_| {}, &token_b_task)
                .await
        });
        tokio::time::sleep(std::time::Duration::from_millis(50)).await;
    ...
    async fn registry_install_rejects_corrupt_downloaded_jar() {
        ...
        let db_type = "h2";
        let version = "0.2.0";
        let jar_url = "https://example.com/chiron-horizon-agent-h2.jar";
        let jar_bytes = b"jar";
    ...
    async fn offline_zip_rejects_a_cross_platform_java_dependency_before_install_changes() {
        ...
        let jar_name = "chiron-horizon-agent-h2.jar";
        let jre_name = format!("chiron-horizon-jre-{jre_key}-{foreign_platform}.tar.gz");
        let jar_bytes = test_agent_jar();
    ...
    fn offline_zip_rejects_a_java_dependency_missing_from_a_registry_with_jre_metadata() {
        ...
        let jar_name = "chiron-horizon-agent-h2.jar";
        let jar_bytes = test_agent_jar();
        let unrelated_jre_name = format!("chiron-horizon-jre-temurin-17-{platform}.tar.gz");
    ...
    fn offline_zip_rejects_registry_size_and_sha256_tampering() {
        let platform = AgentManager::current_platform();
        let entry_name = format!("chiron-horizon-agent-h2-{platform}");
    ...
    fn offline_zip_resolves_a_hyphenated_jre_key_from_the_registry_basename() {
        ...
        let jre_key = "temurin-21";
        let jar_name = "chiron-horizon-agent-h2.jar";
        let jre_name = format!("chiron-horizon-jre-{jre_key}-21.0.7-{platform}.tar.gz");
        let jar_bytes = test_agent_jar();
...
mod jre_dir_remove_tests {
    ...
    fn unique_tmp(name: &str) -> PathBuf {
        std::env::temp_dir().join(format!("chiron-horizon-jre-remove-{name}-{}", uuid::Uuid::new_v4()))
    }
    ...
    fn windows_error_message_lists_root_causes_and_path() {
        let path = PathBuf::from("/tmp/chiron-horizon-jre-test");
        ...
        if cfg!(windows) {
            ...
            assert!(rendered.contains("restart chiron-horizon and try again"), "missing restart advice: {rendered}");
        } else {
