package com.chiron.horizon.agent.dameng;
...
import com.chiron.horizon.agent.test.JdbcMetadataSqlFake;
...
class DamengAgentMetadataTest {
    ...
    void legacyDamengUsesJdbcMetadataWithoutQueryingDm8CatalogViews() {
        ...
        List<TableInfo> tables = agent.listTables("CHIRON_HORIZON_TEST");

        Assertions.assertEquals(List.of("CHIRON_HORIZON_TEST", "SYSDBA"), databases.stream().map(DatabaseInfo::getName).toList());
    ...
    void readsIdentityColumnsFromDdlText() {
        Assertions.assertEquals(
            ...
            DamengAgent.identityColumnsFromDdlText(
                "CREATE TABLE \"CHIRON_HORIZON_TEST\".\"T\"\n(\n\"ID\" INT IDENTITY(1, 1) NOT NULL,\n"
    ...
    void readsFullTableDdlFromCharacterStreamWhenGetStringIsTruncated() {
        ...
        Assertions.assertTrue(ddl.contains("CHIRON_HORIZON_FULL_DDL_END"), ddl);
    }
        ...
        DatabaseMetaData metadata = proxy(DatabaseMetaData.class, (method, args) -> {
            switch (method.getName()) {
                case "getSchemas":
                    ...
                    return metadataResultSet(List.of(List.of("SYSDBA"), List.of("CHIRON_HORIZON_TEST")));
                case "getTableTypes":
